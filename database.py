...
# Files to import (sheet suffix -> json filename)
FILES = [
...
SCHEMAS = {
    "musicDifficulties": [
        "id", "musicId", "musicDifficulty", "playLevel", "totalNoteCount",
    ],
    "musicVocals": [
        "id", "musicId", "musicVocalType", "seq", "releaseConditionId",
        "caption", "characters", "assetbundleName", "archivePublishedAt",
        "specialSeasonId", "archiveDisplayType",
    ],
    "musicTags": [
        "musicId", "musicTag",
    ],
    ...
    "musics": [
        "id", "seq", "releaseConditionId", "categories", "title",
        "pronunciation", "creatorArtistId", "lyricist", "composer",
        "arranger", "dancerCount", "selfDancerPosition", "assetbundleName",
        "publishedAt", "releasedAt", "fillerSec", "isNewlyWrittenMusic",
        "isFullLength", "musicCollaborationId",
    ],
}
    ...
    if isinstance(val, list):
        ...
        for item in val:
            if isinstance(item, dict):
                parts.append("{" + ", ".join(f"{k}:{flatten_value(v)}" for k, v in item.items()) + "}")
            else:
