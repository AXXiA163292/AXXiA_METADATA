...
# difficulty label as it appears in Constants!F
DIFF_LABEL = {"APPEND": "Append", "MASTER": "Master", "EXPERT": "Expert", "HARD": "Hard"}

S = requests.Session()
S.headers.update({"User-Agent": "Mozilla/5.0"})

# ── Load constants sheet (public, no auth needed) ─────────────────────────────
def load_constants():
    ...
    rows = list(reader)
    # row[0]=C(constant), row[1]=D, row[2]=E, row[3]=F(diff), row[4]=G(song_id)
    lookup = {}
    for row in rows[1:]:  # skip header
        ...
        diff     = row[3].strip()
        sid      = row[4].strip()
        if not constant or not diff or not sid: continue
        try: lookup[(diff, int(sid))] = constant
        except ValueError: pass
    print(f"  Constants loaded: {len(lookup)} entries")
    return lookup

def get_constant(constants, song_id, difficulty):
    """Look up constant value; returns '-' if not found."""
    if song_id is None: return "-"
    diff_label = DIFF_LABEL.get(difficulty, difficulty)
    try: return constants.get((diff_label, int(song_id)), "-")
...
def parse_difficulty(difficulty):
    url = "https://pjsekai.com/?" + requests.utils.quote("楽曲難易度表" + difficulty)
    try:
        resp = S.get(url, timeout=30)
    ...
    section_starts = [{"index": m.start(), "lvl": m.group(1)} for m in anchor_re.finditer(html)]
    ...
    for s in range(len(section_starts) - 1):
        ...
        for row_m in re.finditer(r"<tr>([\s\S]*?)</tr>", tbody, re.I):
            ...
            # judgment + history
            judgment = history = ""
            memo1_m = re.search(r'<div[^>]*class="memo1"[^>]*>([\s\S]*?)</div>', cells[1], re.I)
            if memo1_m:
                memo1_html = memo1_m.group(1)
                memo2_m = re.search(r'<span[^>]*class="memo2"[^>]*>([\s\S]*?)</span>', memo1_html, re.I)
                if memo2_m:
            ...
            # elements
            elements = ", ".join(
                strip_tags(li) for li in re.findall(r"<li[^>]*>([\s\S]*?)</li>", cells[2] if len(cells) > 2 else "", re.I)
                if strip_tags(li)
            )
...
# ── Load jp_musics id map from the target spreadsheet ────────────────────────
def load_jp_musics_ids(gc, spreadsheet_id):
    """Returns dict: title -> song_id  (jp_musics!E -> jp_musics!A)"""
    ws = gc.open_by_key(spreadsheet_id).worksheet("jp_musics")
    data = ws.get("A:E")
    mapping = {}
    for row in data[1:]:  # skip header
        if len(row) < 5: continue
        sid   = row[0].strip()   # col A
        title = row[4].strip()   # col E
        if title and sid:
            mapping[title] = sid
...
def JUDGMENT():
    ...
    if not all_rows:
        print("[error] No data scraped."); sys.exit(1)
    ...
    # Resolve constant value for each row
    final_rows = []
    for row in all_rows:
        title      = row[0]
        difficulty = row[4]
        song_id    = id_map.get(title)
        constant   = get_constant(constants, song_id, difficulty) if title != "-" else "-"
        final_rows.append(row + [constant])

    # Get or create judgment tab
    ...
    # Single write
    header = [["title", "judgment", "elements", "comments", "difficulty", "lvl", "history", "39sConstant"]]
    ws.update(range_name="A1", values=header + final_rows, value_input_option="RAW")
