...
def FANDOM():
  ...
  for row in song_rows:
    ...
    if ib:
      ...
      if panel:
        for ti, block in enumerate(
            panel.find_all('div', class_=re.compile(r'wds-tab__content'))[:5]
        ):
          for lbl in block.find_all(class_=re.compile(r'pi-data-label')):
            if lbl.get_text(strip=True).upper() == 'APPEND':
              ...
              if v:
                append[ti] = normalize_date(clean(v))
    ...
    def fix_artist(a):
      ...
      a = a.replace(
          'Project SEKAI x Ensemble Stars', 'Project SEKAI × Ensemble Stars!!'
      )
      a = re.sub(r'25 ji,|25-ji,', '25-ji', a)
      a = re.sub(r'Nightcord de\b(?!\.)', 'Nightcord de.', a)
      a = a.replace(
          'Kyujitsu, Shumijin Doushi de.', 'Kyujitsu Shumijin Doushi de.'
      )
      return a
    ...
    if ah:
      ...
      if tbl:
        for tr in tbl.find_all('tr'):
          ...
          version = re.sub(
              r'Full\/Game Size', 'Full/Game Version', version, flags=re.I
          )
          version = re.sub(
              r'^Game(?!\s+Version)(\s+-)', r'Game Version\1', version
          )

          parts = version.split(' - ')
          ver_type = (
              parts[0].strip().replace('(', '').replace(')', '').strip()
          )
          artist_raw = (
              ' - '.join(parts[1:]).strip().replace('(', '').replace(')', '').strip()
              if len(parts) > 1
              else ''
          )
    ...
    non_instrumental_indices = [
        ...
        if e['versionType']
        not in ('Instrumental', 'Menu Instrumental', 'Menu Instrumental - New')
    ]
    versions_map = {}
    ...
    if vh:
      ...
      if vs_parent:
        ...
        while nxt:
          ...
          if not nxt:
            break
          if nxt.name and re.match(r'h[12]', nxt.name):
            break
          tab_contents += (
              nxt.find_all('div', class_=re.compile(r'wds-tab__content'))
        ...
        for vti, block in enumerate(tab_contents):
          ...
          orig_idx = (
              non_instrumental_indices[vti]
              if vti < len(non_instrumental_indices)
              else None
          )
          if orig_idx is not None:
            versions_map[orig_idx] = {
                'unit': unit2 or 'OTHER',
    ...
    for e in audio_sorted:
      ...
      is_menu_new = (
          vt in ('Menu Instrumental', 'Menu Instrumental - New')
          and e['artist'] == 'New'
      ) or vt == 'Menu Instrumental - New'
      if not is_menu:
        ...
        unit3 = ''
      part = (
          f"{vt};{e['artist']};{e['duration']};{e['url']};{singers2};{unit3}"
      )
      if is_menu_new:
    ...
    if vh:
      ...
      if vs_parent2:
        for a_tag in vs_parent2.find_all(
            'a', class_=re.compile(r'wds-tabs__tab')
        ):
          ver_tab_labels.append(a_tag.get_text(strip=True))
        if not ver_tab_labels:
          for div in vs_parent2.find_all(
              'div', class_=re.compile(r'wds-tabs__tab-label')
          ):
            a2 = div.find('a')
            if a2:
              ver_tab_labels.append(a2.get_text(strip=True))
    for ni, orig_idx2 in enumerate(non_instrumental_indices):
      if orig_idx2 not in used and orig_idx2 in versions_map:
        ...
        tab_label = (
            ver_tab_labels[ni] if ni < len(ver_tab_labels) else '-'
        )
        non_menu_count += 1
        id_str2 = f'{non_menu_count:02d}'
        all_parts.append(
            f"{id_str2};-;{tab_label};-;;{vdata2['singers']};{vdata2['unit'] or 'OTHER'}"
        )
    ...
    if lh:
      ...
      labels = []
      tw = (
          lparent.find_next(class_=re.compile(r'wds-tabs__wrapper'))
          if lparent
          else None
      )
      if tw:
        for a in tw.find_all('a'):
      ...
      if lparent:
        for el in lparent.find_all_next():
          if not isinstance(el, Tag):
            continue
          if el.name and re.match(r'h[12]', el.name):
            break
          if el.attrs is None:
            continue
          if 'poem' in (el.attrs.get('class') or []):
            for span in el.find_all('span', class_=re.compile(r'copy')):
              span.decompose()
